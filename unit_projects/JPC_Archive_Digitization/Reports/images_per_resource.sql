...
from 
	...
	jpc_aspace_data jad 
where 
	(fol.project_id = 201 or fol.project_id = 186 or fol.project_id = 220) and
...
-- update
SELECT max(f.updated_at) as updated_at from files f, folders fol where f.folder_id = fol.folder_id and (fol.project_id = 186 or fol.project_id = 201 or fol.project_id = 220);
