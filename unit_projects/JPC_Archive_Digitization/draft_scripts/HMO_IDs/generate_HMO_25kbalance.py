...
done_refids = ["abdf9b232cdeb401ced7e386d97c013b", "a48abb18653b3f2d7ec1473e63755e6b", "a9e4073af3ac29d2d8601a8c06be5646", "da0c77f6628da9148489b5ee41786cde", "4945c6702e1495b214b24b4bbc71f460"]

get_refids = "SELECT distinct SUBSTRING_INDEX(file_name, '_', 1) as refid FROM files where folder_id in (SELECT folder_id FROM folders WHERE project_id = 186)"
cur.execute(get_refids)
list_refids = cur.fetchall()

for refid in list_refids:
    refid = refid['refid']
    
    if refid in done_refids:
        # If the refid has been ingested already, go to the next
        continue
